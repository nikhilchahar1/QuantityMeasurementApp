...
import com.nikhil.quantitymeasurement.domain.TemperatureUnit;
import com.nikhil.quantitymeasurement.domain.VolumeUnit;
...
class TemperatureTest {
	...
	void testEquality_SameValue_ForAllUnits(TemperatureUnit unit) {
		Quantity<TemperatureUnit> t1 = new Quantity<>(16.0, unit);
		Quantity<TemperatureUnit> t2 = new Quantity<>(16.0, unit);
	...
	void testEquality_DifferentValue_ForAllUnits(TemperatureUnit unit) {
		Quantity<TemperatureUnit> t1 = new Quantity<>(16.0, unit);
		Quantity<TemperatureUnit> t2 = new Quantity<>(22.0, unit);
	...
	void testConstructor_NaN_throws() {
		assertThrows(IllegalArgumentException.class, () -> new Quantity<>(Double.NaN, TemperatureUnit.CELSIUS));
	}

	@Test
	void testConstructor_Infinite_throws() {
		assertThrows(IllegalArgumentException.class,() -> new Quantity<>(Double.POSITIVE_INFINITY, TemperatureUnit.CELSIUS));
	}
